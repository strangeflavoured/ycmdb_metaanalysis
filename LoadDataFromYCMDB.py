...
class LoadDataFromYCMDB:
	def __init__(self):
		import config
	
		#sql connection
		self.path=f"mysql+pymysql://{config.USER}:{config.PASSWORD}@{config.HOST}/{config.DB_NAME}"
	...
	def load_glucose_uptake(self, clean_unit=False, full=False):

		#create mysql connection	
		with create_engine(self.path).connect() as connection:
			...
			data=data.apply(lambda x: self.glucose_uptake_unit(x), axis="columns")
		...
		self.media_table = media_matrix
		self.strain_table = strain_matrix
	...
	def glucose_uptake_unit(self, row, molar_mass=180.156):
		unit=row["Unit"]
	...
	def get_feature_table(self):
		features=pd.merge(self.data_table, self.media_table, how="left", on="Medium_ID")
		features=pd.merge(features, self.strain_table, how="left", on="Strain_ID")
...
if __name__ == "__main__":
	load_data = LoadDataFromYCMDB()
	load_data.load_glucose_uptake()
	feature_table = load_data.get_feature_table()